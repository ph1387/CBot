...
// TODO: UML RENAME ActionUpdaterSimulationQueueTerran
/**
...
public abstract class ActionUpdaterSimulationQueueTerranDefault extends ActionUpdaterSimulationQueue {
	...
	// TODO: UML REMOVE
	// protected HashMap<TypeWrapper, Integer>
	// informationStorageQueuesActionTypes;
	...
	// * provided ActionType <b>must</b> result in a TechType. If this is not
	// the
	// * case, the function will fail. The function tests if the Player has
	// * researched the resulting TechType of the provided {@link ActionType}
	// and
	...
	// * <li>Test if the required UnitType of the stored TechType exists at
	// least
	// * once.</li>
	// * </ul>
	// *
	...
	// */
	// private boolean canResearchTechnology(BuildActionManager manager,
	// ActionType actionType) {
	// Player player = Core.getInstance().getPlayer();
	// boolean notResearched =
	// !player.hasResearched(actionType.defineResultType().getTechType());
	// boolean notResearching =
	// !player.isResearching(actionType.defineResultType().getTechType());
	//
	// boolean requiredTypeExist = this.doesRequiredTypeExist(manager,
	// actionType);
	// boolean notForwardedOrQueued = !this.wasForwardedOrQueued(actionType);
	// boolean idleFacilitiyExists = this.isOneProducingFacilityIdle(manager,
	// actionType);
	// boolean requiredUnitExist = this.doesRequiredUnitExist(manager,
	// actionType.defineResultType().getTechType());
	//
	// return notResearched && notResearching && requiredTypeExist &&
	// notForwardedOrQueued && idleFacilitiyExists
	// && requiredUnitExist;
	// }
	//
	// /**
	...
	// * not yet reached the maximum upgrade level of the resulting UpgradeType
	// of
	// * the provided {@link ActionType} and if he is currently not performing
	// it.
	...
	// * the provided {@link ActionType} are matched, false if not.
	// */
	// private boolean canUpgrade(BuildActionManager manager, ActionType
	// actionType) {
	// Player player = Core.getInstance().getPlayer();
	// boolean maxLevelNotReached =
	// player.getUpgradeLevel(actionType.defineResultType().getUpgradeType()) <
	// player
	// .getMaxUpgradeLevel(actionType.defineResultType().getUpgradeType());
	// boolean notUpgrading =
	// !player.isUpgrading(actionType.defineResultType().getUpgradeType());
	//
	// boolean requiredTypeExist = this.doesRequiredTypeExist(manager,
	// actionType);
	// boolean notForwardedOrQueued = !this.wasForwardedOrQueued(actionType);
	...
	// actionType);
	// boolean requiredUnitExist = this.doesRequiredUnitExist(manager,
	// actionType.defineResultType().getUpgradeType());
	//
	// return maxLevelNotReached && notUpgrading && requiredTypeExist &&
	// notForwardedOrQueued && idleFacilitiyExists
	// && requiredUnitExist;
	// }
	//
	...
	// * the {@link ActionType} (Resulting in a TechType) that is going
	// * to be checked.
	// * @return true if the required UnitType exists at least once, false if
	// not.
	// */
	// private boolean doesRequiredTypeExist(BuildActionManager manager,
	// ActionType actionType) {
	// return
	...
	// * forwarded or is currently in one of the executing / waiting Queues of
	// the
	...
	// return
	// this.simulationQueueResultActionTypes.containsKey(actionType.defineResultType())
	// ||
	...
	// * is found.
	// */
	// private boolean isOneProducingFacilityIdle(BuildActionManager manager,
	// ActionType actionType) {
	// boolean prodcuingFacilityIdles = false;
	//
	// // Find a required, idle and completed facility for the ActionType.
	// for (Unit unit :
	// manager.getInformationStorage().getCurrentGameInformation().getCurrentUnits()
	// .getOrDefault(actionType.defineRequiredType().getUnitType(), new
	// HashSet<Unit>())) {
	...
	// * @return true if the required UnitType of the provided TechType is
	// present
	// * on the map at least once.
	// */
	// private boolean doesRequiredUnitExist(BuildActionManager manager,
	// TechType techType) {
	// boolean exist = true;
	//
	...
	// * present on the map at least once. These are based on the current level
	// of
	// * the UpgradeType and must be matched before upgrading to the next level
	// is
	// * possible.
	// *
	// * @param manager
	...
	// manager.getCurrentGameInformation().getCurrentUpgrades().getOrDefault(upgradeType,
	// 0);
	// boolean exist = true;
	//
	// // Make sure the required UnitType exists at least once and is NOT None!
	// if (currentUpgradeLevel > 0 &&
	// upgradeType.whatsRequired(currentUpgradeLevel + 1) != UnitType.None) {
	// exist &=
	// manager.getInformationStorage().getCurrentGameInformation().getCurrentUnitCounts()
	...
	// * present on the map at least once.
	// */
	// protected boolean doesRequiredUnitExist(BuildActionManager manager,
	// UnitType unitType) {
	// boolean exist = true;
	//
	// for (UnitType requiredUnitType : unitType.requiredUnits().keySet()) {
	// exist &= unitType.requiredUnits().get(requiredUnitType) <=
	// manager.getInformationStorage()
	// .getCurrentGameInformation().getCurrentUnitCounts().getOrDefault(requiredUnitType,
	// 0);
	// }
	//
	// return exist;
	...
	// * researched.
	// */
	// protected boolean doesRequiredTechExist(BuildActionManager manager,
	// UnitType unitType) {
	// boolean exist = true;
	//
	// if (unitType.requiredTech() != TechType.None) {
	// exist &=
	// manager.getCurrentGameInformation().getCurrentTechs().contains(unitType.requiredTech());
	// }
	//
	// return exist;
	// }
